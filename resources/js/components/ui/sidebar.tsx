...
function SidebarProvider({
  ...
  // --- LOGIKA BARU UNTUK INISIALISASI STATE ---
  const [_open, _setOpen] = React.useState(() => {
    // 1. Coba baca state dari localStorage terlebih dahulu
    const storedState = localStorage.getItem(SIDEBAR_STORAGE_KEY);
    if (storedState !== null) {
      // Jika ada, gunakan nilainya
      return storedState === 'true';
    }
    // 2. Jika tidak ada, baru gunakan prop dari server sebagai cadangan
    return defaultOpen;
  });
  // --- SELESAI LOGIKA BARU ---
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // --- UBAH BAGIAN PENYIMPANAN KE LOCALSTORAGE ---
